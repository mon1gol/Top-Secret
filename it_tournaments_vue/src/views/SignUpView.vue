      ...
      <form @submit.prevent="submitForm" class="flex flex-col gap-4">
        <div class="flex gap-3">
          <div>
            ...
            <div class="mt-2">
              <input v-model="first_name" placeholder="Введите" type="text"
          ...
          <div>
            ...
            <div class="mt-2">
              <input v-model="last_name" placeholder="Введите" type="text"
                class="block w-full rounded-xl bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 placeholder:text-sm focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 transition-all duration-150">
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="mt-2">
            <input v-model="password2" placeholder="Введите" type="password"
              class="block w-full rounded-xl bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 placeholder:text-sm focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 transition-all duration-150">
          </div>
        </div>
        <button
          class="mt-7 block w-full rounded-xl bg-blue-three btn-hover-dark z-3 px-3.5 py-2 text-white cursor-pointer">Регистрация</button>
        ...
        <div class="text-center">Или <router-link class="text-blue-700 border-b-1 hover:text-blue-900"
            to="/log-in">войдите</router-link> в аккаунт</div>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    submitForm() {
      ...
      const commonPasswords = ["password", "12345678", "qwerty", "asdfgh"];
      if (commonPasswords.includes(this.password.toLowerCase())) {
        this.errors.push("Пароль слишком простой или распространённый.");
      }

      if (this.password.toLowerCase() === this.first_name.toLowerCase()) {
        this.errors.push("Пароль не должен совпадать с именем пользователя.");
      }
      ...
      if (!/[a-z]/.test(this.password)) {
        this.errors.push("Пароль должен содержать хотя бы одну строчную букву.");
      }
      ...
      if (!/[!@#$%^&*(),.?":{}|<>]/.test(this.password)) {
        this.errors.push("Пароль должен содержать хотя бы один специальный символ.");
      }
